...
interface JinchanWallet {
  postMessage(...args: unknown[]): unknown;
}

interface JinchanWalletWindow extends Window {
  jinchanWallet?: JinchanWallet;
}

declare const window: JinchanWalletWindow;

export interface JinchanWalletAdapterConfig {
  provider?: string | JinchanWallet;
  network?: WalletAdapterNetwork;
  timeout?: number;
}

export abstract class BaseJinchanWalletAdapter extends BaseMessageSignerWalletAdapter {
  protected _provider: string | JinchanWallet | undefined;
  ...
  constructor({
    ...
    if (this._readyState !== WalletReadyState.Unsupported) {
      if (typeof this._provider === "string") {
        ...
        scopePollingDetectionStrategy(() => {
          if (typeof window.jinchanWallet?.postMessage === "function") {
  ...
  async connect(): Promise<void> {
    try {
      ...
      const provider = this._provider || window!.jinchanWallet!;

      console.log({ provider });

      let SolWalletAdapterClass: typeof SolWalletAdapter;
      try {
        ({ default: SolWalletAdapterClass } = await import(
  ...
  async disconnect(): Promise<void> {
    ...
    if (wallet) {
      ...
      const handleDisconnect: (...args: unknown[]) => unknown = (wallet as any)
        .handleDisconnect;
      try {
        await new Promise<void>((resolve, reject) => {
