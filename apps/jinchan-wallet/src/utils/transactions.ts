...
const toInstruction = async (
  ...
  try {
    if (programId.equals(SystemProgram.programId)) {
      ...
      //   return {
      //     type: 'Unknown',
      //     accountMetas: instruction.accounts.map((index: number) => ({
...
//   instruction: { programIdIndex: any; accounts: any; data?: any },
//   accountKeys: any[],
//   decodedInstruction: { [x: string]: any }
// ) => {
//   if (!decodedInstruction || Object.keys(decodedInstruction).length > 1 || !connection) {
//     return;
//   }
...
//     MARKETS.find(
//       (market) =>
//         accountKeys.findIndex((accountKey: { equals: (arg0: PublicKey) => any }) =>
...
//     programIdAddress = marketInfo?.programId || getAccountByIndex([programIdIndex], accountKeys, 0);
...
//   } catch (e) {
//     console.log('Error loading market: ' + (e as any).message);
//   }
...
const handleSystemInstruction = (
  ...
  if (!decoded || ((decoded as any).fromPubkey && !publicKey.equals((decoded as any).fromPubkey))) {
    return;
  }
...
// const getNewOrderV3Data = (accounts: any, accountKeys: any) => {
...
// const getSettleFundsData = (accounts: any, accountKeys: any) => {
